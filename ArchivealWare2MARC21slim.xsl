...
<xsl:stylesheet version="1.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns="http://www.loc.gov/MARC21/slim" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="dc">
	...
	<xsl:template match="/">
      ...
      <xsl:if test="//subject[@values='SuDoc'] != ''">
        <datafield tag="086" ind1=" " ind2=" ">
          <subfield code="a">
            <xsl:value-of select="//subject[@values='SuDoc']/."/>
          </subfield>
        </datafield>
      </xsl:if>

      <xsl:if test="//subject[@values='LCC'] != ''">
        <datafield tag="086" ind1=" " ind2=" ">
          <subfield code="a">
            <xsl:value-of select="//subject[@values='LCC']/."/>
          </subfield>
        </datafield>
      </xsl:if>
			...
			<xsl:choose>
				<xsl:when test="//publisher">
					<xsl:if test="translate(//publisher/.,'.,:;','')!=''">
						<datafield tag="260" ind1=" " ind2=" ">
						<xsl:choose>
							<xsl:when test="//date">	
								<subfield code="b"><xsl:value-of select="//publisher[0]"/>, </subfield>	
								<xsl:if test="translate(//date[0]/., '.,:;','')!=''">
									<subfield code="c"><xsl:value-of select="//date[0]" />.</subfield>
								</xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<subfield code="b"><xsl:value-of select="//publisher[0]"/>.</subfield>	
      ...
      <xsl:if test="//description/tableOfContents != ''">
        <datafield tag="520" ind1=" " ind2=" ">
          <subfield code="a">
            <xsl:value-of select="//description/abstract/."/>
          </subfield>
      ...
      <xsl:for-each select="//rights">
			...
			<xsl:if test="//subject[@values='LCSH'] != ''">
				<datafield tag="650" ind1=" " ind2="0">
					<subfield code="a">
						<xsl:value-of select="//subject[@values='LCSH']/."/>
					</subfield>
				</datafield>
			</xsl:if>

      <xsl:if test="//subject[@values='Other'] != ''">
        <datafield tag="653" ind1=" " ind2=" ">
          <subfield code="a">
            <xsl:value-of select="//subject[@values='Other']/."/>
          </subfield>
        </datafield>
      </xsl:if>
      ...
      <xsl:for-each select="//type">
			...
			<xsl:for-each select="//contributer">
			...
			<xsl:if test="identifier">
				<datafield tag="856" ind1="4" ind2="1">
					...
					<subfield code="u"><xsl:value-of select="identifier[last()]" /></subfield>
