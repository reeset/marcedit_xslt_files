...
<xsl:stylesheet version="1.0"  xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  >
  ...
  <xsl:template match="DISS_submission">
		<marc:record>
		  ...
		  <marc:leader>02211ntm a2200289Ia 4500</marc:leader>
		  <marc:controlfield tag="006"><xsl:text>m\\\\\\\\d\s\\\\\\</xsl:text></marc:controlfield>
		  <marc:controlfield tag="007"><xsl:text>cr\|n|||||||||</xsl:text></marc:controlfield>
		  
		  <marc:controlfield tag="008">
		    <xsl:text>051012s</xsl:text>
		    <xsl:value-of select="$date1" />
		    <xsl:text>    xx a     bm o 000 0 eng d</xsl:text>
		  </marc:controlfield>
        ...
        <marc:datafield tag="300" ind1=" " ind2=" ">
          <marc:subfield code="a">1 online resource (<xsl:value-of select="$page" /> p.) ; </marc:subfield>
          <marc:subfield code="c">29 cm.</marc:subfield>
        </marc:datafield>
		  </xsl:if>
		  
		  <marc:datafield tag="500" ind1=" " ind2=" ">
		    <marc:subfield code="a">Printout.</marc:subfield>
		  </marc:datafield>
		  ...
		  <marc:datafield tag="502" ind1=" " ind2=" ">
		    ...
		    <marc:subfield code="a">Thesis (<xsl:value-of select="$degree" />) -- <xsl:value-of select="$org" />, <xsl:value-of select="$date1" />.</marc:subfield>
		  </marc:datafield>
		  ...
		  <xsl:if test="DISS_content/DISS_abstract/DISS_para">
		    <xsl:variable name="abtmp" select="normalize-space(DISS_content/DISS_abstract/DISS_para)" />
		    <marc:datafield tag="520" ind1="3" ind2=" ">
		      <xsl:if test="string-length($abtmp)&lt;9980">
		        <marc:subfield code="a">
		          <xsl:value-of select="$abtmp" />
		        </marc:subfield>  
		      </xsl:if>
		      
		      <xsl:if test="string-length($abtmp)&gt;9980">
		        <marc:subfield code="a">
		          <xsl:value-of select="substring($abtmp, 1, 9980)" />...
		        </marc:subfield>
		      </xsl:if>
		    </marc:datafield>
		  </xsl:if>
		  ...
		  <!-- Need to deal with Subjects and Keywords -->
		  <xsl:for-each select="DISS_description/DISS_categorization/DISS_category">
		    <marc:datafield tag="690" ind1=" " ind2=" ">
		      <marc:subfield code="a"><xsl:value-of select="DISS_cat_desc" /></marc:subfield>
		    </marc:datafield>  
		  </xsl:for-each>
		  ...
		  <marc:datafield tag="856" ind1="4" ind2="1">
		    <marc:subfield code="u"><xsl:value-of select="DISS_description/@external_id" /></marc:subfield>
		  </marc:datafield>
...
<xsl:template name="persname_template">
  ...
  <xsl:template name="init_subject_template">
    <xsl:param name="string" />
    <xsl:param name="field" />
    <xsl:param name="ind1" />
    <xsl:param name="ind2" />
    <xsl:choose>
      <xsl:when test="contains($string, ' ;')">
        <xsl:variable name="data" select="substring-before($string, ' ;')" />
        ...
        <xsl:if test="contains($newstring, ' ;')">
          <xsl:call-template name="init_subject_template">
