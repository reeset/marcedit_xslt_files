...
<xsl:stylesheet version="1.0" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/ead">
		<marc:collection xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
			...
			<xsl:call-template name="primary_record" />
			<xsl:for-each select="//dsc/c01">
				<xsl:call-template name="analytic_record" />
			</xsl:for-each>
			<xsl:for-each select="//dsc/c01/c02[@level='item']">
				<xsl:call-template name="analytic_record" />
			</xsl:for-each>
		</marc:collection>
	</xsl:template>
	<xsl:template name="primary_record">
		<marc:record>
			<marc:leader>
	...
	<xsl:template name="analytic_record">
			...
			<marc:record>
				...
				<xsl:if test="did/unittitle">
					<marc:datafield tag="245" ind1="1" ind2="0">
						...
						<xsl:if test="did/unitdate">
							<xsl:choose>
								<xsl:when test="did/unitdate/@encodinganalog='245$g'">
									<xsl:if test="did/unitdate/@encodinganalog='245$f'">
										<marc:subfield code="f">
											<xsl:value-of select="normalize-space(did/unitdate[@encodinganalog='245$f']/.)" />
										</marc:subfield>
									</xsl:if>
									<xsl:if test="did/unitdate/@encodinganalog='245$g'">
										<marc:subfield code="g">bulk <xsl:value-of select="normalize-space(did/unitdate[@encodinganalog='245$g']/.)" />).</marc:subfield>
									</xsl:if>
								</xsl:when>
								<xsl:otherwise>
									<xsl:if test="did/unitdate/@encodinganalog='245$f'">
										<marc:subfield code="f"><xsl:value-of select="normalize-space(did/unitdate[@encodinganalog='245$f']/.)" />.</marc:subfield>
									</xsl:if>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:if>
				...
				<marc:datafield tag="300" ind1=" " ind2=" ">
					<marc:subfield code="a">boxes (count of boxes c02)</marc:subfield>
				</marc:datafield>
				...
				<xsl:choose>
					<xsl:when test="did/scopecontent">
						<marc:datafield tag="520" ind1="2" ind2=" ">
							<marc:subfield code="a">
					...
					<xsl:otherwise>
						<xsl:if test="did/abstract">
							<xsl:if test="did/abstract/@encodinganalog='5203_'">
								<marc:datafield tag="520" ind1="3" ind2=" ">
									<marc:subfield code="a">
										<xsl:value-of select="normalize-space(did/abstract/p/.)" />
									</marc:subfield>
								</marc:datafield>
							</xsl:if>
						</xsl:if>
				...
				<xsl:if test="//prefercite">
					<marc:datafield tag="524" ind1=" " ind2=" ">
						<marc:subfield code="a">
							<xsl:value-of select="normalize-space(//prefercite/.)" />
						</marc:subfield>
					</marc:datafield>
				</xsl:if>
